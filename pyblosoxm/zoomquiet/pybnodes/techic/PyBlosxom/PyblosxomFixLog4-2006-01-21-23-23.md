\[hack4\]SVN集成 通过SVN来发布Blog t2t渲染:2024-07-27
...
:   -   曾经\[透过CVS进行内容维护
        <http://wiki.woodpecker.org.cn/moin/CvsRootHook>\]
    -   后来同理推导出:[透过SVN进行内容维护
...
-   一般性的，PyBlosxom 是从原文头行取，但是 xhtml 中有HTML
    的格式标识字串，所以，要从\`\`\<title\>\`\` 标签中抓，
...
但是:
:   不能聪明的识别不同的原始文件来提供下载哪！ 如果是 .py 的，在
    *永久链接 .t2t .moin* 处的其它链接就有问题！
    -   询问列表，却没有人回答:
